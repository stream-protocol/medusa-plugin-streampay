...
class StreamUSDCPaymentService extends AbstractPaymentService {
  ...
  async getPaymentData(paymentSession: PaymentSession): Promise<Data> {
    ...
    streamPayment.solana_wallet_addr = this.wallet.publicKey.toString();
    await this.streamPaymentRepository.save(streamPayment);

    return {
      "paymentAddress": streamPayment.solana_wallet_addr,
    };
  }

  // Implement other methods for Solana transactions and interactions
