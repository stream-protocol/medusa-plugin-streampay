...
class StreamSOLPaymentService extends AbstractPaymentService {
  ...
  private async connect() {
    if (this.streamDaemon == null) {
      ...
      // Create SOL (Solana-based) wallet
  ...
  async getPaymentData(paymentSession: PaymentSession): Promise<Data> {
    ...
    streamPayment.sol_wallet_addr = this.wallet.publicKey.toString();
    await this.streamPaymentRepository.save(streamPayment);

    return {
      "paymentAddress": streamPayment.sol_wallet_addr,
    };
  }

  // Implement other methods for SOL (Solana) transactions and interactions

}

export default StreamSOLPaymentService;
